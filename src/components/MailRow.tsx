      ...
      <ContextMenuTrigger>
        <div
          ...
          className={cn(
            ...
            unread
              ? "font-[450] text-[#302a28]  dark:text-white bg-[rgba(0,0,0,0.025)] hover:!bg-[rgba(0,0,0,0.04)] dark:!bg-[rgba(255,255,255,0.015)] dark:hover:!bg-[rgba(255,255,255,0.06)]"
              : "text-gray-700/90 dark:text-gray-300",
            className
          )}
        >
          <div className="w-[75%] flex flex-1 items-center gap-4">
            ...
            <div className="min-w-[130px] w-[18%]">
              <p className="w-full overflow-hidden">
                <span title={filterdFromName[0].email}>
                  {getUserName(
                    filterdFromName[0].name,
                    filterdFromName[0].email
                  )}
                </span>
                ...
                {filterdFromName.length > 1 && (
                  <>
                    ...
                    <span title={filterdFromName[1].email}>
                      {getUserName(
                        filterdFromName[1].name,
                        filterdFromName[1].email
                      )}
                    </span>
                  </>
                )}
                ...
                {count > 1 && (
                  <span className="text-sm ml-1 !text-[13px] text-gray-400 dark:text-gray-300">
                    {" "}
                    {count}
                  </span>
                )}
            ...
            <p className="max-w-[60%] flex items-center whitespace-nowrap overflow-hidden">
              {unread && (
                <div className="min-w-[10px] h-[10px] bg-core rounded-full mr-3"></div>
              )}
              {subject ? (
                (() => {
                  ...
                  return (
                    <div className="flex gap-2">
                      {sub.tags?.map((s) => (
                        <span className="bg-button-hover px-1 rounded-sm">
                          {s}
                        </span>
                      ))}
                      {sub.cleanSubject}
              ...
              {count == 1 && text && text != "" && (
                <span className="text-gray-500 dark:text-gray-300/70 overflow-hidden text-ellipsis">
                  &nbsp;-{" "}
                  {text.split("\n")[0].split(" ").splice(0, 7).join(" ")}...
                </span>
              )}
            </p>
          </div>
